...
from train_cartpole_cleanrl_ddqn import QNetwork


rom_time_check = True
env = gym.make("CartPole-v1", render_mode="human")
network_env = gym.vector.SyncVectorEnv([lambda: gym.make("CartPole-v1")])
checkpoint = torch.load("rom_cleanrl_ddqn_cartpole.cleanrl_model", map_location="cpu")
q_network = QNetwork(network_env, use_dueling=checkpoint.get("use_dueling", False))
network_env.close()
