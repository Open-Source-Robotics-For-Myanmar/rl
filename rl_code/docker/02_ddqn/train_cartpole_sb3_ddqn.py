...
def epsilon_schedule(step: int, total_timesteps: int) -> float:
	progress = min(step / max(total_timesteps * 0.5, 1), 1.0)
...
def parse_args() -> argparse.Namespace:
	parser = argparse.ArgumentParser(description="Train DDQN with the SB3 replay buffer.")
...
def train(args: argparse.Namespace) -> None:
	...
	run_name = f"CartPole-v1__sb3_ddqn__{args.seed}__{int(time.time())}"
	writer = SummaryWriter(Path("sb3_ddqn_cartpole_tensorboard") / run_name)
	...
	for global_step in range(args.total_timesteps):
		epsilon = epsilon_schedule(global_step, args.total_timesteps)
	...
	model_path = Path("rom_sb3_ddqn_cartpole.cleanrl_model")
	torch.save({"env_id": "CartPole-v1", "model_state_dict": q_network.state_dict()}, model_path)
